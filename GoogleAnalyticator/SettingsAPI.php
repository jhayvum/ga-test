...
//			$values['desc']	 = '<div class="inside">' . $values['desc'] . '</div>';
	...
	public function display() {
		...
		?>
<div id="<?php echo $this->_args['id']; ?>" class="wrap">
	<h2><?php echo $this->_args['title']; ?></h2>
	<?php
	if ( $this->_args['tabbed'] ) {
		$this->tab_nav();
	}
	// <form id="yoast-ga-form-settings" class="yoast_ga_form" method="post" action="/wp-admin/admin.php?page=yst_ga_settings">
	// '<form action="' . $action . '" method="post" id="yoast-ga-form-' . $this->form_namespace . '" class="yoast_ga_form">
	?>
